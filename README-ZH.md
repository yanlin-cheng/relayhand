...
relayhand 干的是"把工作**交出当前会话**"这件事，下面这些场景有更顺手的工具：

- **同一个任务、同一个会话，只是变重了** → `/compact`。原地压缩就是对的选择，换会话是杀鸡用牛刀。
- **任务做完了，或者下一棒从零开始** → 直接开个新对话（`/clear`）。没什么可交接的。
- **工作必须离开这个会话——或者跨到另一个 AI 产品** → 这才是 relayhand 的主场。
...
## 建议技能    ← 下一棒该调用的技能（Claude Code 版专属）
